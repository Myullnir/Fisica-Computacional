...
int main(){
	...
	time(&prin);
	float Be,protran;
	int i,j,n,si,sf,l=32,nb=10,medcorr=200000, termal=100000,tardanza; //Atento que quizás cambiar el nb te da problemas con campob
    int nmed = 4*medcorr;
    int *li;
	...
	*li=0;
	...
	for(i=0;i<nb;i++) *(campob+i) = 0.1+0.1*i;
	int *grilla;
	...
	for(n=7;n<8;n++){
		...
		for(j=0;j<10;j++){
			...
			/*sprintf(filename,"Datos_Ising_J=0_termj=%d_Be=%f.txt",j,Be);
