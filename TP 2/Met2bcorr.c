// Vamos a calcular la correlacion para el punto 2b aca
...
int main(){
	...
	time(&prin);
	float Je;
	int i,j,n,f,c,si,sf,su,sd,sr,sl,l=32,nj=6,medcorr=300000,termal=1000000,tardanza; //El nj es 6, ya hice la cuenta y no va a cambiar. Por ahora
	int nmed=3*medcorr;
	int *li;
	...
	*li=0;
	...
	for(n=0;n<nj;n++){
		...
		for(j=0;j<6;j++){
			...
			Correlacion(energia,j,nmed,medcorr,Je);
		}
		time(&fin);
...
int Correlacion(float *muestreo, int j, int nmed, int medcorr, float d){
	...
	sprintf(filename,"Datos_Correlacion_Ising_Energia_Medicion=%d_Je=%f.txt",j,d);
