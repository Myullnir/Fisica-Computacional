// Vamos a calcular la termalización para el punto 2c aca
...
int main(){
	...
	time(&prin);
	float Je;
	int i,j,n,m,p=-1,f,c,si,sf,su,sd,sl,sr,l,nj=6,tardanza; //El nj es 6, ya hice la cuenta y no va a cambiar. Por ahora
	int *li;
	li = (int*) malloc(1*sizeof(int));
	*li=0;
	int *nmed;
	nmed = (int*) malloc(5*sizeof(int));
	*nmed = 2000;
	*(nmed+1) = 3000;
	*(nmed+2) = 100000;
	*(nmed+3) = 2000000;
	*(nmed+4) = 5000000;
	int *muestras;
	muestras = (int*) malloc(5*sizeof(int));
	*muestras = 100;
	*(muestras+1) = 30;
	*(muestras+2) = 15;
	*(muestras+3) = 6;
	...
	for(l=4;l<=8;l=l*2){
		...
		for(n=0;n<nj;n++){
			...
			for(j=0;j<*(muestras+p);j++){
				...
				sprintf(filename,"Datos_Ising_B=0_Termj=%d_L=%d_Je=%f.txt",j,l,Je);
				FILE *fp=fopen(filename,"w");
				/*fprintf(fp,"Magnetismo\t");
				for(m=0;m<nmed+1;m++) fprintf(fp,"%f\t",*(magnet+m));
				fprintf(fp,"\n");*/
				fprintf(fp,"Energia\t");
				for(m=0;m<*(nmed+p)+1;m++) fprintf(fp,"%f\t",*(energia+m));
				fclose(fp);
			}
			time(&fin);
