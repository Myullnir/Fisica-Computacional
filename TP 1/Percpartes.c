...
#define SEMILLA 0; //Esta es la que voy a usar para mi numero random
...
int main()
	...
	int *canti;
	canti=(int*) malloc((((dim*dim)/2)+1)*sizeof(int));
	//printf("Arme historial\n");	
	int i;
	for(i=0; i<((dim*dim)/2)+1; i++)
		{//printf("Estoy armando el historial");
		*(historial+i)=i;
		*(taman+i)=0;
		*(canti+i)=0;
		}
