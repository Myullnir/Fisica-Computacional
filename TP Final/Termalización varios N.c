...
int main(){
	...
	for(int d=4; d<1000; d = d*2){
		datos.N2 = d+2; // Largo de la red
		...
		char filename[255];
		sprintf(filename,"Termalizacion_Red_N=%d.txt",d);
		...
		time(&fin);
		tardanza = fin-prin;
		printf("Tarde %d segundos en guardar los datos de la red de tamaño %d\n",tardanza,d);
	}
	return 0;
}
