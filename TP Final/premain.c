...
int main(){
	...
	time_t prin,fin;
	time(&prin);
	int i,j,l,N=512,pasos=300,termal=500,desco=20,Nlut,tardanza; //lg
	double rho,T=1.5,T2,crit,rc=2.5,h=0.001,dr=0.001,L;// deltar=0.05;
	Nlut = (int) ceil(rc/dr);
	double *x;
	...
	for(l=0;l<3;l++){ 
		rho = 0.6+l*0.2;
		sprintf(filename,"Datos_gr_D=%lf.lammpstrj",rho);
		// No voy a armar este archivo, total no voy a guardar la gr
		...
		L = set_x(x,rho,N);
		set_v(v,T,N);
		// Como estoy guardando el lammps, estos no hacen falta
		...
		for(i=0;i<pasos;i++){
			...
			save_lammpstrj(filename, x, v, N, L, i);
			//distg(x,gr,L,rho,deltar,N,lg);
			//for(j=0;j<lg;j++) *(grP+j) += *(gr+j)/pasos;
			
		}
		// Ahora voy a guardar un archivo lammps, así que no es necesario esto
